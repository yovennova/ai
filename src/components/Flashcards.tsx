...
export default function Flashcards() {
  ...
  const handleGenerate = async () => {
    ...
    setIsLoading(true);
    try {
      const langInstruction = language === "am" ? " Please generate the flashcards in Amharic." : "";
      const result = await generateFlashcards(text + langInstruction);
      setCards(result || []);
      setCurrentIdx(0);
      setIsFlipped(false);
    } catch (error) {
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col gap-2">
        <h2 className="text-3xl font-bold text-slate-900 tracking-tight">{t.flashcardsTitle}</h2>
            ...
            >
              {isLoading ? (
                <>
                  ...
                  <span>{t.generatingFlashcards}</span>
                </>
              ) : (
                <>
                  <Sparkles size={20} />
                  <span>{t.generateFlashcards}</span>
                </>
              )}
            </button>
          </div>
        ...
        <div className="max-w-2xl mx-auto space-y-10">
          <div className="flex items-center justify-between px-2">
            <span className="text-sm font-semibold text-indigo-600 uppercase tracking-wider">
              {t.question} {currentIdx + 1} {t.outOf} {cards.length}
            </span>
            <button 
              onClick={() => setCards([])}
              className="text-sm text-slate-500 hover:text-indigo-600 flex items-center gap-1 transition-colors cursor-pointer"
            >
              <RotateCcw size={14} />
              <span>{t.reset}</span>
            </button>
          ...
          <div className="relative h-96 perspective-1000">
            ...
            >
              ...
              <div className="absolute inset-0 backface-hidden bg-white border-2 border-slate-200 rounded-[2.5rem] shadow-xl flex flex-col items-center justify-center p-12 text-center group">
                <div className="absolute top-6 left-6 text-xs font-bold text-slate-300 uppercase tracking-widest">{t.front}</div>
              ...
              >
                <div className="absolute top-6 left-6 text-xs font-bold text-indigo-300 uppercase tracking-widest">{t.back}</div>
          ...
          <div className="flex items-center justify-center gap-6">
            <button
              ...
              className="w-14 h-14 bg-white border border-slate-200 rounded-full flex items-center justify-center text-slate-600 hover:bg-slate-50 hover:border-indigo-200 hover:text-indigo-600 disabled:opacity-30 disabled:cursor-not-allowed transition-all shadow-sm cursor-pointer"
            >
              <ChevronLeft size={24} />
            </button>
            <button
