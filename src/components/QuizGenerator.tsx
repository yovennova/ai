...
export default function QuizGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    setIsLoading(true);
    try {
      const langInstruction = language === "am" ? " Please generate the quiz in Amharic." : "";
      const result = await generateQuiz(text + langInstruction);
      setQuestions(result || []);
      setCurrentIdx(0);
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col gap-2">
        <h2 className="text-3xl font-bold text-slate-900 tracking-tight">{t.quizTitle}</h2>
      ...
      {!questions.length ? (
        ...
        >
          ...
          <div className="space-y-2">
            <h3 className="text-3xl font-bold text-slate-900">{t.quizCompleted}</h3>
            <p className="text-slate-500">{t.score} {score} {t.outOf} {questions.length}</p>
          </div>
          ...
          <button
            onClick={resetQuiz}
            className="px-8 py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-semibold flex items-center justify-center gap-2 mx-auto transition-all shadow-lg shadow-indigo-100 cursor-pointer"
          >
            <RefreshCw size={20} />
            <span>{t.tryAnotherQuiz}</span>
          </button>
        </motion.div>
      ) : (
        <div className="max-w-3xl mx-auto space-y-8">
          <div className="flex items-center justify-between px-2">
            <span className="text-sm font-semibold text-indigo-600 uppercase tracking-wider">
              {t.question} {currentIdx + 1} {t.outOf} {questions.length}
            </span>
          ...
          >
              ...
              {questions[currentIdx].options.map((option, idx) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "w-full p-5 rounded-2xl text-left border-2 transition-all flex items-center justify-between group cursor-pointer",
                      !selectedOption 
                        ? "border-slate-100 hover:border-indigo-200 hover:bg-indigo-50/30 text-slate-800" 
                        : isSelected
                          ? isCorrect ? "border-green-500 bg-green-50 text-green-900" : "border-red-500 bg-red-50 text-red-900"
                          : isCorrect ? "border-green-500 bg-green-50 text-green-900" : "border-slate-100 opacity-50 text-slate-800"
                    )}
                  >
            ...
            <AnimatePresence>
              {selectedOption && (
                <motion.div 
                  ...
                  className="p-6 bg-indigo-50 rounded-2xl border border-indigo-100 space-y-2 text-left"
                >
                  <p className="text-sm font-bold text-indigo-900 uppercase tracking-wider">{t.explanation}</p>
            ...
            <div className="flex justify-end pt-4">
              <button
                ...
                disabled={!selectedOption}
                className="px-8 py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-semibold flex items-center gap-2 disabled:opacity-50 transition-all shadow-lg shadow-indigo-100 cursor-pointer"
              >
                <span>{currentIdx === questions.length - 1 ? t.finishQuiz : t.nextQuestion}</span>
